export async function getSkillModifiers(interaction, env) {
  ...
  let dbResult = await env.DB.prepare(
    'SELECT * FROM SkillModifiers WHERE PlayerName = ?',
  )
    .bind(playerName)
    .run();
...
export async function getSkillBonus(playerName, skill, env) {
  let dbResult = await env.DB.prepare(
    'SELECT * FROM SkillModifiers WHERE PlayerName = ?1 AND Skill = ?2',
  )
    .bind(playerName, skill)
    .run();
  if (dbResult.results && dbResult.results.length > 0) {
    return dbResult.results[0].Modifier;
  }
}
...
export async function setSkillModifier(interaction, env) {
  ...
  if (dbResult.results.length === 0) {
    await env.DB.prepare(
      'INSERT INTO SkillModifiers(Modifier, PlayerName, Skill) VALUES (?1,?2,?3)',
    )
      .bind(modifier, playerName, skill)
      .run();
  } else {
    await env.DB.prepare(
      'UPDATE SkillModifiers SET Modifier = ?1 WHERE PlayerName = ?2 AND Skill = ?3',
    )
      .bind(modifier, playerName, skill)
      .run();
  }
...
export async function deleteSkillModifier(interaction, env) {
  ...
  if (dbResult.results.length === 0) {
    return (
      'This skill has not been registered for ' +
      playerName +
      ' and therefor can not be deleted.'
    );
  }
  await env.DB.prepare(
    'DELETE FROM SkillModifiers WHERE PlayerName = ?1 AND Skill = ?2',
  )
    .bind(playerName, skill)
    .run();
...
export async function deleteAllSkills(interaction, env) {
  ...
  if (dbResult.results.length === 0) {
    return (
      'There are no skills registered for ' +
      playerName +
      ' and therefor nothing was deleted.'
    );
  }
  await env.DB.prepare('DELETE FROM SkillModifiers WHERE PlayerName = ?1')
    .bind(playerName)
    .run();
